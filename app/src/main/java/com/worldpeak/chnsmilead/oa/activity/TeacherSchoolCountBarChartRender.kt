...
class TeacherSchoolCountBarChartRender(
    private val uiParams: TeacherSchoolCountChartUiParams,
    ...
    override fun drawDataSet(c: Canvas, dataSet: IBarDataSet, index: Int) {
        ...
        while (j < buffer.size()) {
            ...
            c.drawRoundRect(
                ...
                buffer.buffer[j + 3], uiParams.barRound, uiParams.barRound, mRenderPaint
            )
            if (drawBorder) {
                c.drawRect(
                    buffer.buffer[j], buffer.buffer[j + 1], buffer.buffer[j + 2],
