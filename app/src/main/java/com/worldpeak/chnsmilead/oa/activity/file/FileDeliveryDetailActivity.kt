...
import com.worldpeak.chnsmilead.databinding.ActivityFileDeliveryDetailBinding
...
import com.worldpeak.chnsmilead.oa.viewmodel.file.FileDeliveryDetailViewModel
import com.worldpeak.chnsmilead.util.*

/**
 * 文件下发详情
 */
class FileDeliveryDetailActivity : BaseVmVBActivity<FileDeliveryDetailViewModel, ActivityFileDeliveryDetailBinding>() {

    private val mFileList: MutableList<CommonAccessory> = mutableListOf()
    private val mFileAdapter = MultiTypeAdapter().apply {
        register(CommonAccessory::class.java, DeliveryDetailFileLongViewBinder(this@FileDeliveryDetailActivity) {
    ...
    private val mAdapter = MultiTypeAdapter().apply {
        register(SchoolNotifyInfo::class.java, DeliverySchoolViewBinder(this@FileDeliveryDetailActivity) {
            val intent = Intent(this@FileDeliveryDetailActivity, FileDeliveryReplyListActivity::class.java)
            intent.putExtra(FileDeliveryReplyListActivity.EXTRA_FORMID, mInfo?.formId)
    ...
    private val mInfo by lazy {
        intent.getParcelableExtra(EXTRA_INFO) as? FileDeliveryItem
    }
    ...
    override fun getLayoutId(): Int {
        return R.layout.activity_file_delivery_detail
    }
    ...
    override fun initView() {
                    ...
                    val list = mutableListOf<ListItemInfo>().apply {
                        add(ListItemInfo(content = "发出", color = "#4EBBF5", click = View.OnClickListener {
                            mViewModel.addFileDelivery(
                                    ...
                                    mResponse?.papersDowmAccessoryList?: emptyList<CommonAccessory>()
                            )
                        }))
                        add(ListItemInfo(content = "删除", click = View.OnClickListener {
                            mViewModel.deleteFileDelivery(mutableListOf<String?>().apply { add(mInfo?.formId) })
                        }))
                    }
                    ListSelectDialog(this@FileDeliveryDetailActivity, list, true).show()
                }
            }
            1 -> {
                mBinding.titleBar.showRightIcon()
                mBinding.titleBar.setRightIconClick {
                    val list = mutableListOf<ListItemInfo>().apply {
                        add(ListItemInfo(content = "撤回通知", click = View.OnClickListener {
                            mViewModel.rebackFileDelivery(mInfo?.formId)
                        }))
                    }
                    ListSelectDialog(this@FileDeliveryDetailActivity, list, true).show()
                }
        ...
        mBinding.tvRemind.setPreventDoubleClickListener {
            CommonConfirmDialog(this@FileDeliveryDetailActivity, "将再次发送此通知，提醒未确认老师", "取消", "发送", {}, {
                mViewModel.remindFileNotConfirm(mResponse?.formId)
            }).show()
        }
    }
    ...
    private var mResponse: FileDeliveryDetailResponse? = null
    ...
    override fun loadData() {
        ...
        mViewModel.result.observe(this, Observer { response ->
            ...
            if (response.papersDowmAccessoryList.isNullOrEmpty()) {
                ...
                mFileList.clear()
                response.papersDowmAccessoryList?.getOrNull(0)?.let { it1 -> mFileList.add(it1) }
                ...
                mBinding.tvLookAll.setPreventDoubleClickListener {
                    mBinding.tvLookAll.hide()
                    mFileList.clear()
                    mFileList.addAll(response.papersDowmAccessoryList)
